...
| `$A` `$B` | **jump**: move the whole region to a new random offset each step, within 50% / 100% of the wrap domain |
...
- **JUMP** is ROL with a thrown step: one offset for the whole region, uniform over `±round(domain_length × fraction)` (`$B` over the whole domain) and wrapped into it. It **MUST NOT** accumulate — each step's offset is measured from the region's real position, not added to the last one, or `$A` random-walks into `$B` inside a few seconds and the pair stops being a pair. Every byte moves together, so the waveform is intact when it lands; this is a randomised `O $xxyy`, once a step, and it is the one random operation that leaves the instrument recognisable.
