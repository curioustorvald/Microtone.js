...
- **A third theme: dim, and it is the new default.** The ◐ button now cycles dark → dim → light rather than flipping between two. Dim is a neutral grey rather than the dark theme lightened — the blue cast is gone, which is what tells the two apart at a glance — and it keeps the same ink and accents, so nothing moves or changes meaning when you switch. Everyone gets it on a first visit now, whatever the desktop is set to; if you have ever picked a theme here, your choice is untouched. It applies to the documentation viewer too, and `?theme=dim` in the URL selects it.
